  ...
  ifit('Auto Eat', 
    ...
    (done) => {
      ...
      bot.instance.once('spawn', async () => {
        ...
        const food_before = bot.instance.food;
        await autoEat(bot.instance, {
          food: ['cooked_porkchop'],
          threshold:{
            food:19,
            health:19
          },
          onlyQuickBarSlot: true,
          offhand: true
        });
        
        const food_after = bot.instance.food;
        expect(food_before != undefined && food_after != undefined && 
          (food_before == 20 || food_before < food_after)).toBeTruthy();
